...
# Deine bestehenden Definitionen
...
# Widgets
heatmap_period = widgets.Dropdown(options=time_periods.keys(), description='Zeitraum:')
heatmap_season = widgets.RadioButtons(options=['Summer', 'Winter'], description='Saison:')
heatmap_country = widgets.Dropdown(options=region_options, description='Land:')
heatmap_gender = widgets.Dropdown(options=['Alle', 'M', 'F'], description='Geschlecht:')
heatmap_button = widgets.Button(description='Heatmap anzeigen', button_style='info')

def plot_heatmap(period, season, country, gender):
    ...
    display(heatmap_period, heatmap_season, heatmap_country, heatmap_gender, heatmap_button)

    if df_filtered.empty:
        print(f"⚠️ Keine Medaillendaten für {country} ({season}, {period}, Geschlecht: {gender})")
        return

    # Pivot-Tabelle: Sportart vs Jahr (Zählung Medaillen)
    heatmap_data = (
        df_filtered
        .groupby(['sport', 'year'])
        .size()
    ...
    sns.heatmap(heatmap_data, annot=True, fmt='d', cmap='YlOrBr', linewidths=0.5, cbar_kws={'label': 'Medaillen'})
    plt.title(f"Medaillenheatmap – {country} ({season} {period}, Geschlecht: {gender})")
    plt.xlabel("Jahr")
    plt.ylabel("Sportart")
    plt.tight_layout()
    plt.show()
...
def on_heatmap_click(b):
    plot_heatmap(
        heatmap_period.value,
        heatmap_season.value,
        heatmap_country.value,
        heatmap_gender.value
    )
